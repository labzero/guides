...
For working with a rails app, please include `gem scss_lint, require: false` in your Gemfile and then run `bundle install`. For configuration, please read and reference the default yml file we have for basic setup[SCSS YML](https://github.com/languages/css/.scss.yml) and [SCSS-LINT](https://github.com/labzero/corpsite/blob/master/Gemfile).  Finally when all is set up run either `scss-lint` in your root directory to fix issues locally. Before making a PR please include scss-lint in your `ci.rake` file, an example can be found here, [LZ Corpsite ci.rake](https://github.com/labzero/corpsite/blob/master/lib/tasks/ci.rake).  Notice, you must include the scss run command in `task build` && a task block inside of `namespace :ci`. 
